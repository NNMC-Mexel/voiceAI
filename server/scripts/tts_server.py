...
# ---------------------------------------------------------------------------
# Медицинские аббревиатуры → полная форма для корректного произношения
# ---------------------------------------------------------------------------
_ABBREVS: list[tuple[str, str]] = [
    # Единицы измерения
    (r'°[СC]',              ' градусов Цельсия'),
    (r'%',                  ' процентов'),
    (r'мм\s*рт\.?\s*ст\.?', 'миллиметров ртутного столба'),
    (r'уд/мин',             'ударов в минуту'),
    (r'уд\./мин',           'ударов в минуту'),
    (r'мл/мин',             'миллилитров в минуту'),
    (r'г/л',                'граммов на литр'),
    (r'мг/дл',              'миллиграммов на децилитр'),
    (r'мкмоль/л',           'микромолей на литр'),
    (r'ммоль/л',            'миллимолей на литр'),
    (r'нг/мл',              'нанограммов на миллилитр'),
    (r'мкг/л',              'микрограммов на литр'),
    # Показатели
    (r'\bSpO2\b',           'сатурация'),
    (r'\bSaO2\b',           'сатурация'),
    (r'\bЧСС\b',            'частота сердечных сокращений'),
    (r'\bЧД\b',             'частота дыхания'),
    (r'\bАД\b',             'артериальное давление'),
    (r'\bИМТ\b',            'индекс массы тела'),
    # Исследования
    (r'\bЭКГ\b',            'электрокардиограмма'),
    (r'\bЭХОКГ\b',          'эхокардиограмма'),
    (r'\bЭЭГ\b',            'электроэнцефалограмма'),
    (r'\bМРТ\b',            'магнитно-резонансная томография'),
    (r'\bКТ\b',             'компьютерная томография'),
    (r'\bУЗИ\b',            'ультразвуковое исследование'),
    (r'\bОАК\b',            'общий анализ крови'),
    (r'\bОАМ\b',            'общий анализ мочи'),
    (r'\bБАК\b',            'биохимический анализ крови'),
    (r'\bСОЭ\b',            'скорость оседания эритроцитов'),
    (r'\bСРБ\b',            'С-реактивный белок'),
    (r'\bАЛТ\b',            'аланинаминотрансфераза'),
    (r'\bАСТ\b',            'аспартатаминотрансфераза'),
    # Диагнозы
    (r'\bИБС\b',            'ишемическая болезнь сердца'),
    (r'\bОИМ\b',            'острый инфаркт миокарда'),
    (r'\bХСН\b',            'хроническая сердечная недостаточность'),
    (r'\bХОБЛ\b',           'хроническая обструктивная болезнь лёгких'),
    (r'\bСД\b',             'сахарный диабет'),
    (r'\bГБ\b',             'гипертоническая болезнь'),
    (r'\bОНМК\b',           'острое нарушение мозгового кровообращения'),
    (r'\bТИА\b',            'транзиторная ишемическая атака'),
    (r'\bВБН\b',            'вертебробазилярная недостаточность'),
    (r'\bЖКБ\b',            'желчнокаменная болезнь'),
    (r'\bГПОД\b',           'грыжа пищеводного отверстия диафрагмы'),
    (r'\bМТС\b',            'метастазы'),
    # Способ введения
    (r'\bв/в\b',            'внутривенно'),
    (r'\bв/м\b',            'внутримышечно'),
    (r'\bп/к\b',            'подкожно'),
]

# Компилируем паттерны один раз при старте
...
def _preprocess(text: str) -> str:
    """Очистить markdown и раскрыть медицинские аббревиатуры."""
    # 1. Убрать markdown-форматирование
    text = re.sub(r'\*\*(.+?)\*\*', r'\1', text)          # **жирный**
    text = re.sub(r'\*(.+?)\*', r'\1', text)               # *курсив*
    text = re.sub(r'__(.+?)__', r'\1', text)               # __подчёркнутый__
    text = re.sub(r'#{1,6}\s+', '', text)                  # ## заголовок
    text = re.sub(r'`{1,3}[^`]*`{1,3}', '', text)         # `код`
    text = re.sub(r'^\s*[-*]\s+', '', text, flags=re.M)   # - список → убрать маркер
    text = re.sub(r'^\s*\d+\.\s+', '', text, flags=re.M)  # 1. список → убрать номер
    text = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', text)  # [ссылка](url)

    # 2. Раскрыть аббревиатуры
