...
- `server/.env` содержит секреты (`ANTHROPIC_API_KEY`, `AUTH_PASSWORD`) и в репозиторий не уходит (см. корневой `.gitignore`). На проде использовать секрет-менеджер (Vault / AWS Secrets Manager / Doppler), не хранить ключи в plaintext.
- `STRUCTURE_LOG_DUMP=true` включает запись raw-транскрипта Whisper + структурированного JSON в `temp/structure-logs/`. Содержит медданные пациентов — включать только для отладки, по умолчанию выключено.
- `ALLOW_MOCK_LLM=true` включает graceful-fallback к пустому документу при отказе LLM. На проде должен быть `false`, чтобы сбой структурирования возвращал клиенту явную ошибку, а не «успешный» пустой результат.
