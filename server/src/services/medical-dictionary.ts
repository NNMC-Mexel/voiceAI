...
const PHONETIC_CORRECTIONS: ReplacementRule[] = [
  ...
  // "матче спускания" / "матчеспускание" → "мочеиспускание"
  ...
  // "аускультативное в лёгких" → "Аускультативно: в лёгких"
  regexRule(/аускультативное\s+в\s+легких/giu, 'Аускультативно: в лёгких'),
  // "аортасклероз" → "атеросклероз аорты" (склеенные слова)
  regexRule(/аортасклероз/giu, 'атеросклероз аорты'),
  ...
  // Whisper мусор: URL-подобные конструкции посреди текста
  regexRule(/,?\s*site\s+\S*https?:?\S*[^.]*\./giu, '.'),
  regexRule(/,?\s*https?:\/\/\S+/giu, ''),
  // Whisper мусор: английские слова посреди русского текста (3+ англ. слова подряд)
  regexRule(/,?\s*(?:[a-z]+\s+){3,}[a-z]+\S*/giu, ''),
  // Whisper мусор: кириллица+латиница смешанные слова (Сниженfl, последdatedo и т.п.)
  regexRule(/\s+[а-яёА-ЯЁ]+[a-zA-Z]+\S*/gu, ''),
  regexRule(/\s+[a-zA-Z]+[а-яёА-ЯЁ]+\S*/gu, ''),
  // Whisper мусор: "permanent образом" — отдельные англ. слова перед русскими
  regexRule(/(?<!\S)(?:permanent|tachy|site|prince|mentioned|detective|verific)\s*/giu, ''),
