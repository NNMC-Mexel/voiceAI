...
const VITALS_FORMATTING: ReplacementRule[] = [
  ...
  // Также: "АД 144/97" → "АД – 144/97 мм рт.ст." (если без "мм рт.ст.")
  regexRule(
    /(?<![а-яёa-z])АД\s*[-–—]?\s*(\d{2,3})\s+на\s+(\d{2,3})(?!\s*(?:мм\s*рт|\/\d))/giu,
    'АД – $1/$2 мм рт.ст.'
  ),
  // "АД 144/97" без единиц → добавляем единицы
  regexRule(
    /(?<![а-яёa-z])АД\s*[-–—]?\s*(\d{2,3})\/(\d{2,3})(?!\s*мм\s*рт)/giu,
    'АД – $1/$2 мм рт.ст.'
  ),
  // "давление 144 на 97" (если "артериальное давление" уже заменено на АД, но текст другой)
  regexRule(
    /давлени\S*\s+(\d{2,3})\s+на\s+(\d{2,3})/giu,
    'АД – $1/$2 мм рт.ст.'
  ),
