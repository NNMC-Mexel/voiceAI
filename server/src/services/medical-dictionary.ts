...
const PHONETIC_CORRECTIONS: ReplacementRule[] = [
  ...
  regexRule(/кожан\S+\s+покров/giu, 'кожные покров'),
  // "дилетация" → "дилатация"
  wordRule('дилетация', 'дилатация'),
  wordRule('дилетационная', 'дилатационная'),
  // "сирадиация" → "с иррадиацией"
  ...
  // "штутного столба" / "ртутного столба" без "мм" → "мм рт.ст." (Whisper искажение)
  ...
  // "трикоспидальная" → "трикуспидальная"
  wordRule('трикоспидальная', 'трикуспидальная'),
  wordRule('трикоспидальный', 'трикуспидальный'),
  ...
  // "тахибродикардитический" → "тахи-брадикардитический"
  regexRule(/тахибродикардитическ/giu, 'тахи-брадикардитическ'),
  ...
  // "РСДЛА" → "РСДЛА" (оставляем, это правильная аббревиатура)
  // "кардиоресинхронизирующего" → "кардиоресинхронизирующего" (оставляем)
  ...
  // Whisper галлюцинации в конце аудио — мусор с латиницей/кириллица+латиница
  // Ищем первое слово где кириллица склеена с латиницей и обрезаем всё после
...
const VITALS_FORMATTING: ReplacementRule[] = [
  ...
  regexRule(
    /давлени\S*\s+(?:(?:до|от|при)\s+)?(\d{2,3})\s+на\s+(\d{2,3})\s*(?:мм\s*рт\.?\s*ст\.?)?/giu,
    'АД – $1/$2 мм рт.ст.'
  ),
